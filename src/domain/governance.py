...
from src.models.article import Article, ArticleTag, ArticleUserPermission, ArticleVersion, DocumentSource
...
class GovernanceService:
    ...
    async def _get_draft_for_user(self, draft_id: uuid.UUID, user: User, *, for_update: bool = False) -> PendingDraft | None:
    ...
    async def log_audit(self, user_id: uuid.UUID | None, action: str, target_type: str, target_id: str | None = None, outcome: str = "success") -> AuditLog:
        log = AuditLog(
            user_id=user_id,
            action=action,
    ...
    async def _transition_draft(self, draft: PendingDraft, actor: User | None, to_status: str, reason: str | None = None) -> None:
        allowed = {
        ...
        if to_status not in allowed.get(draft.status, set()):
            raise HTTPException(status_code=409, detail=f"Draft cannot transition from {draft.status} to {to_status}")
        db = self.gov_repo.db
        if hasattr(db, "add"):
            db.add(DraftTransition(
                draft_id=draft.id,
    ...
    def _unmapped_external_acl_principals(metadata: dict) -> list[str]:
        ...
        principals = [
            *(f"group:{item}" for item in metadata.get("unmapped_group_ids", []) or [] if item),
            *(f"user:{item}" for item in metadata.get("unmapped_source_user_ids", []) or [] if item),
            *(str(item) for item in metadata.get("unmapped_principal_ids", []) or [] if item),
        ]
    ...
    async def list_drafts(self, user: User, status: str | None = None) -> Sequence[PendingDraft]:
        ...
        if can_company_review:
            ...
            if self._is_company_governance_lead(user):
                return await self.gov_repo.list_drafts(status, user.company_domain, assigned_approver_id=user.id)
            ...
            member_department_ids = {
                ...
                and getattr(department, "company_domain", user.company_domain) == user.company_domain
            }
    ...
    async def submit_draft(self, user: User, draft_id: uuid.UUID, reason: str | None = None) -> PendingDraft:
        ...
        derived_candidate = (getattr(draft, "content_metadata", None) or {}).get("submission_kind") == "split_candidate"
        if draft.created_by != user.id and not self._is_global_publisher(user) and not (derived_candidate and self._can_review_draft(user, draft)):
            raise HTTPException(status_code=403, detail="Only the draft author or a global publisher can submit this draft")
        if draft.status != "draft":
            raise HTTPException(status_code=409, detail=f"Draft cannot be submitted from status: {draft.status}")
        await self._transition_draft(draft, user, "pending", reason or "Submitted for independent approval")
        rule_loader = getattr(self.gov_repo, "get_approver_rule", None)
        rule = await rule_loader(draft.company_domain, draft.dept) if rule_loader else None
        if rule and rule.active:
            approver = await UserRepository(self.gov_repo.db).get_by_id(rule.approver_id, viewer=user)
            if approver and approver.active and approver.company_domain == draft.company_domain and self._can_review_draft(approver, draft):
                if not draft.created_by or approver.id != draft.created_by or self._may_self_approve(approver):
                    ...
                    self.gov_repo.db.add(NotificationQueue(
                        ...
                        type="in_app",
                        payload={"event": "draft_assigned_by_rule", "draft_id": str(draft.id), "approver_id": str(approver.id), "assigned_by": str(user.id)},
                    ))
                    self.gov_repo.db.add(AuditLog(user_id=user.id, action="assign_approver_rule", target_type="draft", target_id=str(draft.id)))
        updated = await self.gov_repo.update_draft(draft)
        await self.log_audit(user.id, "submit", "draft", str(draft.id))
        return updated

    async def list_candidates(self, user: User, draft_id: uuid.UUID) -> Sequence[DraftCandidate]:
        ...
        return await loader(draft_id, user) if loader else list(getattr(draft, "candidates", []) or [])

    async def review_candidate(self, user: User, draft_id: uuid.UUID, operation: str, candidate_id: uuid.UUID, other_candidate_id: uuid.UUID | None = None, title: str | None = None, split_at: int | None = None, note: str | None = None) -> Sequence[DraftCandidate]:
        draft = await self._get_draft_for_user(draft_id, user, for_update=True)
        if not draft:
            raise HTTPException(status_code=404, detail="Draft not found")
        if draft.status != "pending":
            raise HTTPException(status_code=409, detail="Only pending documents can be batch reviewed")
        if not self._can_review_draft(user, draft):
            raise HTTPException(status_code=403, detail="Not authorized to batch review this document")
        ...
        if operation == "rename":
            if not title or not title.strip():
                raise HTTPException(status_code=422, detail="A candidate title is required")
            ...
            if not other or other.status != "candidate" or other.id == candidate.id:
                raise HTTPException(status_code=422, detail="Choose another active candidate to merge")
            ...
            second.review_note = (note or f"Merged into candidate {first.position}")[:2000]
        elif operation == "split":
            boundary = split_at or len(candidate.body_md) // 2
            if boundary < 1 or boundary >= len(candidate.body_md):
                raise HTTPException(status_code=422, detail="The split position must be inside the candidate text")
            for item in sorted((item for item in candidates if item.status == "candidate" and item.position > candidate.position), key=lambda item: item.position, reverse=True):
                item.position += 1
            ...
            raise HTTPException(status_code=422, detail="Unsupported candidate operation")
        await self.gov_repo.db.commit()
        await self.log_audit(user.id, f"candidate_{operation}", "draft_candidate", str(candidate.id))
        return await self.list_candidates(user, draft_id)

    async def commit_candidates(self, user: User, draft_id: uuid.UUID) -> list[PendingDraft]:
        parent = await self._get_draft_for_user(draft_id, user, for_update=True)
        if not parent:
            raise HTTPException(status_code=404, detail="Draft not found")
        if parent.status != "pending":
            raise HTTPException(status_code=409, detail="Only pending documents can be committed")
        if not self._can_review_draft(user, parent):
            raise HTTPException(status_code=403, detail="Not authorized to commit batch candidates")
        candidates = [item for item in await self.list_candidates(user, draft_id) if item.status == "candidate"]
        if not candidates:
            raise HTTPException(status_code=422, detail="Keep at least one candidate before committing")
        db = self.gov_repo.db
        ...
        for item in sorted(candidates, key=lambda value: value.position):
            child_hash = hashlib.sha256(f"{parent.source_hash}:{item.position}:{item.body_md}".encode("utf-8")).hexdigest()
            metadata = {
                **base_metadata,
                "submission_kind": "split_candidate",
                "source_position": {"start": item.source_start, "end": item.source_end, "heading": item.heading},
            }
            child = PendingDraft(
                title=item.title,
            ...
            db.add(child)
            await db.flush()
            db.add(DraftTransition(draft_id=child.id, from_status=None, to_status="draft", actor_id=user.id, reason="Created from batch-reviewed candidate", outcome="applied"))
            children.append(child)
            item.status = "committed"
        await self._transition_draft(parent, user, "rejected", "Split candidates committed as independent drafts")
    ...
    async def assign_approver(self, user: User, draft_id: uuid.UUID, approver_id: uuid.UUID | None = None, use_rule: bool = False) -> PendingDraft:
        draft = await self._get_draft_for_user(draft_id, user)
        if not draft:
            raise HTTPException(status_code=404, detail="Draft not found")
        if draft.status != "pending":
        ...
        if use_rule:
            ...
            if not rule or not rule.active:
                raise HTTPException(status_code=404, detail="No active approver rule exists for this department")
            approver_id = rule.approver_id
        if not approver_id:
            raise HTTPException(status_code=422, detail="approver_id is required unless use_rule is true")

        approver = await UserRepository(self.gov_repo.db).get_by_id(approver_id, viewer=user)
        ...
        if draft.created_by and approver.id == draft.created_by and not self._may_self_approve(approver):
        ...
        await self.gov_repo.db.commit()
        await self.log_audit(user.id, "assign_approver_rule" if use_rule else "assign_approver", "draft", str(draft.id))
        return updated
    ...
    async def approve_draft(self, user: User, draft_id: uuid.UUID, category: str | None = None, dept: str | None = None, update_article_id: uuid.UUID | None = None, treat_as_new: bool = False, sensitivity: str | None = None, access_group_ids: list[uuid.UUID] | None = None, department_ids: list[uuid.UUID] | None = None, review_note: str | None = None, visibility: str | None = None, explicit_user_ids: list[uuid.UUID] | None = None, denied_user_ids: list[uuid.UUID] | None = None) -> Article:
        ...
        try:
            ...
            visibility = visibility or str(metadata.get("visibility") or ("public" if sensitivity == "public" else "department"))
            if explicit_user_ids is None:
                try:
                    explicit_user_ids = [uuid.UUID(str(user_id)) for user_id in metadata.get("explicit_user_ids", [])]
                except (TypeError, ValueError) as exc:
                    raise HTTPException(status_code=422, detail="The submitted explicit-user selection is invalid") from exc
            if denied_user_ids is None:
                try:
                    denied_user_ids = [uuid.UUID(str(user_id)) for user_id in metadata.get("denied_user_ids", [])]
                except (TypeError, ValueError) as exc:
                    raise HTTPException(status_code=422, detail="The submitted explicit-deny selection is invalid") from exc
            ...
            if not self._can_approve_draft(user, draft):
                raise HTTPException(status_code=403, detail="Only the assigned approver or an Admin can approve this draft")
            external_document = None
            if draft.external_document_id:
                external_document = await db.get(ExternalDocument, draft.external_document_id)
                if not external_document:
                    raise HTTPException(status_code=409, detail="The connector source record is missing; approval is blocked")
                unresolved_acl = self._unmapped_external_acl_principals(external_document.metadata_json or {})
                if unresolved_acl:
                    raise HTTPException(
                        status_code=409,
                        detail={
            ...
            active_candidates = [
                item for item in (getattr(draft, "candidates", []) or [])
                if item.status == "candidate"
            ]
            if len(active_candidates) > 1:
                raise HTTPException(
                    status_code=409,
                    detail={"code": "batch_review_required", "message": "Review and commit split candidates before approving this document"},
                )
            ...
            try:
                submitted_department_ids = department_ids if department_ids is not None else metadata.get("department_ids", [])
                department_ids = [uuid.UUID(str(item)) for item in submitted_department_ids]
            except (TypeError, ValueError) as exc:
            ...
            if visibility not in {"public", "department", "users"}:
                raise HTTPException(status_code=422, detail="Invalid Article visibility")
            ...
            if set(explicit_user_ids) & set(denied_user_ids):
                raise HTTPException(status_code=422, detail="A user cannot be both explicitly allowed and denied")
            selected_explicit_users = list(await UserRepository(db).get_by_ids(explicit_user_ids + denied_user_ids, draft.company_domain))
            if len(selected_explicit_users) != len(set(explicit_user_ids + denied_user_ids)):
                raise HTTPException(status_code=422, detail="Every explicit Article user must belong to the Article company")
            if visibility == "users" and not explicit_user_ids:
                raise HTTPException(status_code=422, detail="Explicit-user visibility requires at least one user")
            ...
            if sensitivity != "public" and not selected_groups and not explicit_user_ids and not draft.external_document_id:
                sensitivity = "public"
            ...
            if external_document:
                ...
                try:
                    external_source_user_ids = {
                        uuid.UUID(str(item)) for item in external_metadata.get("mapped_source_user_ids", [])
                    }
                except (TypeError, ValueError) as exc:
                    raise HTTPException(status_code=422, detail="The connector ACL contains an invalid mapped user") from exc
                selected_external_users = list(await UserRepository(db).get_by_ids(list(external_source_user_ids), draft.company_domain))
                if len(selected_external_users) != len(external_source_user_ids):
                    raise HTTPException(status_code=422, detail="The connector ACL contains a user outside this company")
                explicit_user_ids = list(dict.fromkeys([*explicit_user_ids, *external_source_user_ids]))
                if set(explicit_user_ids) & set(denied_user_ids):
                    raise HTTPException(status_code=422, detail="A connector-mapped user cannot be explicitly denied")
                selected_explicit_users = list(await UserRepository(db).get_by_ids(explicit_user_ids + denied_user_ids, draft.company_domain))
                if len(selected_explicit_users) != len(set(explicit_user_ids + denied_user_ids)):
                    raise HTTPException(status_code=422, detail="Every explicit Article user must belong to the Article company")
            ...
            if update_article_id:
                update_target = await self.article_repo.get_by_id_for_update(update_article_id, user=user) if hasattr(self.article_repo, "get_by_id_for_update") else await self.article_repo.get_by_id(update_article_id, user=user)
            ...
            if update_target:
                ...
                permission_state_before = (
                    ...
                    tuple(sorted(str(department.id) for department in getattr(created_article, "departments", []))),
                    str(created_article.sensitivity or ""),
                    str(created_article.visibility or "department"),
                    tuple(sorted(str(group.id) for group in getattr(created_article, "access_groups", []))),
                    tuple(sorted(
                ...
                connector_permissions = [
                    item for item in getattr(created_article, "user_permissions", [])
                    if item.source == "sharepoint"
                ]
                source_allow_ids = {item.user_id for item in connector_permissions if item.effect == "allow"}
                connector_permissions.extend(
                    ArticleUserPermission(user_id=external_user_id, effect="allow", source="sharepoint")
                    for external_user_id in external_source_user_ids
                    if external_user_id not in source_allow_ids
                )
                created_article.user_permissions = connector_permissions + [
                ...
                permission_state_after = (
                    str(created_article.dept or ""),
                    tuple(sorted(str(department.id) for department in selected_departments)),
                    str(created_article.sensitivity or ""),
                    str(created_article.visibility or "department"),
                ...
                if permission_state_before != permission_state_after:
                    db.add(AuditLog(user_id=user.id, action="permission_change", target_type="article", target_id=str(created_article.id)))
            else:
                created_article = Article(
                    ...
                    lifecycle_status="active", last_reviewed=datetime.utcnow(), related_article_ids=draft.related_article_ids, source_position=metadata.get("source_position"), access_groups=selected_groups,
                    visibility=visibility,
                    user_permissions=[
                        ArticleUserPermission(
                            user_id=explicit_user_id,
                            effect="allow",
                            source="sharepoint" if explicit_user_id in external_source_user_ids else None,
                        )
                        for explicit_user_id in explicit_user_ids
                    ] + [
                        ArticleUserPermission(user_id=denied_user_id, effect="deny")
            ...
            db.add(ArticleVersion(article_id=created_article.id, version=next_version, snapshot={
                ...
                "visibility": created_article.visibility, "explicit_user_ids": [str(item) for item in explicit_user_ids], "denied_user_ids": [str(item) for item in denied_user_ids],
    ...
    async def restructure_draft(self, user: User, draft_id: uuid.UUID, enabled: bool = True) -> PendingDraft:
        ...
        if not self._can_review_draft(user, draft):
            raise HTTPException(status_code=403, detail="Only users with review or publish permission can restructure this draft")
    ...
    async def decide_restructure(self, user: User, draft_id: uuid.UUID, decision: str) -> PendingDraft:
        """Let an authorized reviewer choose the AI candidate or safe fallback."""
        draft = await self._get_draft_for_user(draft_id, user)
        if not draft:
            raise HTTPException(status_code=404, detail="Draft not found")
        if draft.status != "pending":
            raise HTTPException(status_code=400, detail="Only pending drafts can change the reading-view decision")
        if draft.company_domain != user.company_domain and not self._is_global_publisher(user):
            raise HTTPException(status_code=403, detail="Draft is outside your company")
        ...
        if not self._can_review_draft(user, draft):
            raise HTTPException(status_code=403, detail="You do not have approval permission for this draft")
        if decision not in {"keep_ai", "keep_lossless"}:
            raise HTTPException(status_code=422, detail="Decision must be keep_ai or keep_lossless")

        if decision == "keep_ai":
            if not draft.restructure_candidate_md:
                raise HTTPException(status_code=409, detail="There is no retained AI candidate to keep")
            ...
            draft.restructured_body_md = _fallback_text(draft.summary or draft.restructured_body_md or "")
    ...
    async def reject_draft(self, user: User, draft_id: uuid.UUID, review_note: str) -> PendingDraft:
        ...
        if draft.company_domain != user.company_domain and not self._is_global_publisher(user):
            raise HTTPException(status_code=403, detail="Draft is outside your company")
        if not self._can_approve_draft(user, draft):
            raise HTTPException(status_code=403, detail="Only the assigned approver or an Admin can reject this draft")
